...
const NavItem = ({ item, level, pathDirect, hideMenu, onClick }: ItemType) => {
  ...
  return (
    <List component="li" disablePadding key={item?.id && item.title}>
      <Link href={item.href}>
        <ListItemStyled disabled={item?.disabled} selected={pathDirect === item?.href} onClick={lgDown ? onClick : undefined}>
          <ListItemIcon
            sx={{
              minWidth: "36px",
              p: "3px 0",
          ...
          {!item?.chip || (!hideMenu && <Chip color={item?.chipColor} variant={item?.variant ? item?.variant : "filled"} size="small" label={item?.chip} />)}
        </ListItemStyled>
      </Link>
    </List>
  );
